...
- [ ] **`services/llm.py` に `structure_candidate(raw_text) -> CandidateParseResult` を追加**する（`structure_job` と同構造。既存の汎用 `structured_chat`（リトライ付き）を再利用し、応募書類用の system/user プロンプトを差し替える）。
...
- [ ] **テストを追加**する（`test_candidates_api.py` / `test_llm.py` 拡張）。Ollama はモック化（既存 `conftest.py` の autouse モックを利用）。
...
- [ ] backend テスト（Ollama はモック。docs/03_how/06_testing.md 準拠）で以下をカバーする:
  ...
  - `structure_candidate`: `structured_chat` を期待プロンプト・スキーマで呼び、結果を `CandidateParseResult` にマッピングする（`test_llm.py`）。
...
- [backend/services/llm.py](../backend/services/llm.py) — `structured_chat` / `structure_job`（再利用・踏襲元）
