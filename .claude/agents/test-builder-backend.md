...
diff を読み、**変更・追加されたファイル・関数・クラス**を列挙する。これが今回のテストスコープ。diff に含まれないコードへの新規テスト追加は原則しない（既存テストの修正は別）。

次に、diff の変更内容と「テストの優先順」を照合し、**今回の変更に該当する観点だけ**を選ぶ:

- 新規 API エンドポイント / ステータスコード変更 → **API 契約**
- バックグラウンドタスク / 状態フィールド追加 → **ステータス遷移**
- LLM 呼び出しパス変更 → **失敗系 + LLM モック**
- スコア計算 / ランキングロジック → **境界・欠損**
- 構造化データの変換・保存パス → **構造化結果の通り道**

該当しない観点のテストは今回の作業範囲外として触れない。

### Step 1: 既存テストの流儀を掴む

`backend/tests/` を Read/Grep し、命名・フィクスチャ・モックのスタイルを確認する。**既存テストがあれば新規作成せず Edit で拡張**（二重実装を作らない）。

### Step 2: テストを足す/直す

Step 0 で選んだ観点に絞って書く。正常系だけでなく**失敗系・境界**を含める。決定的なテストにする（時刻・乱数・ネットワーク依存で落ちない）。
...
既存テストを壊していないこと（リグレッション）まで確認する。

### Step 4: 完了報告

**追加/変更したテストファイル**・各検証の結果（pass/fail）・**今回カバーした観点**・diff 範囲で意図的にスキップした観点とその理由を簡潔に伝える。
